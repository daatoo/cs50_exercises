...
def index():
    ...
    cash = cash_db[0]["cash"]
    return render_template("index.html", database = transactions_db, cash = format(cash, '.2f'))
...
def buy():
    ...
    if request.method == "GET":
        ...
        if isinstance(shares, float):
            return apology("Shares Must Not Be Float")
        if not symbol:
        ...
        if stock == None:
            return alopogy("Symbol Does Not Exist")
        ...
        new_user = db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)", user_id, stock["symbol"], shares,  format(stock["price"], '.2f'), date)

        flash("Bought! You Purchase " + str(usd(transaction_value)))
        return redirect("/")
@app.route("/history")
@login_required
def history():
...
def sell():
    ...
    if request.method == "GET":
        ...
        flash("You Sold" + str(usd(transaction_value)))
        return redirect("/")